...
export default function Messager() {
  ...
  return (
    <div className={`h-screen overflow-hidden ${pageBg} ${mainText} p-4 lg:p-6 ${FONT_FAMILY} transition-colors duration-300 flex flex-col`}>
...
function ChatPanel({ targetUser, currentUser, onClose, theme }: { targetUser: Tradesman, currentUser: Tradesman, onClose: () => void, theme: any }) {
  ...
  return (
    <div className={`${theme.cardStyle} rounded-xl h-full flex flex-col overflow-hidden animate-in slide-in-from-right-4 duration-300`}>
      {/* Chat Header */}
      <div className={`p-4 border-b ${theme.isDark ? "border-gray-700" : "border-gray-200"} flex justify-between items-center shrink-0`}>
        <div className="flex items-center gap-3">
          <div className={`h-10 w-10 rounded-full overflow-hidden flex items-center justify-center bg-gray-500`}>
          ...
          <div>
            ...
            <span className={`text-xs ${theme.subText} flex items-center gap-1`}>
              {targetUser.Position || "พนักงาน"}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={onClose} className={`p-2 rounded-full hover:bg-opacity-10 hover:bg-gray-500 ${theme.subText}`}>
            <X size={18} />
          </button>
        </div>
      </div>
      ...
      {/* Chat Messages */}
      <div ref={scrollRef} className={`flex-1 overflow-y-auto p-4 space-y-4 overscroll-contain ${theme.isDark ? "bg-gray-900/50" : "bg-gray-50/50"}`}>
        {loadingChat ? (
            <div className="h-full flex items-center justify-center">
                <Loader2 className={`animate-spin ${theme.subText}`} />
            </div>
        ) : messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center opacity-40 text-sm">
            <p>เริ่มการสนทนา</p>
          </div>
        ) : (
          messages.map((msg) => (
      ...
      <div className={`p-3 border-t ${theme.isDark ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"} shrink-0`}>
        <div className={`flex items-center gap-2 px-4 py-2 rounded-full border transition-colors ${theme.isDark ? "border-gray-600 bg-gray-900 focus-within:border-yellow-500" : "border-gray-300 bg-gray-50 focus-within:border-blue-500"}`}>
          <input 
            type="text" 
            value={inputValue}
          ...
          <button 
            onClick={handleSend}
            className={`p-1.5 rounded-full transition-all ${inputValue.trim() ? `${theme.primaryText} hover:bg-opacity-10 hover:bg-gray-500` : "text-gray-400 cursor-not-allowed"}`}
          >
            <Send size={18} />
          </button>
        </div>
