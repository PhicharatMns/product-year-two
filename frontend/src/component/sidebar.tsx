...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {showManu && (
        <div
          className={` fixed lg:left-68 left-4 bottom-0 rounded-t-xl h-150 w-80 p-2 bg-white lg:z-50 transition-all transform   ${
            slideIn
              ? "-translate-x-4 scale-100 opacity-100"
              : " -translate-x-40 scale-100 opacity-0"
          }`}
        >
          ...
          <div>
            <div
              onClick={() => setTimeout(() => setManage(true), 10)}
      ...
      {openManage && (
        <div
          className={` fixed lg:left-150 left-4 bottom-0 rounded-t-xl h-150 w-80 p-2 bg-white lg:z-50 transition-all transform   ${
            slideIn
              ? "-translate-x-4 scale-100 opacity-100"
              : " -translate-x-4 scale-100 opacity-0"
          }`}
        >
          <div className="border-black p-2 rounded-xl flex items-center gap-3">
            ...
            <div className="ml-auto">
              <button className="border text-2xl p-1 cursor-pointer">✕</button>
            </div>
          </div>
        </div>
      )}
