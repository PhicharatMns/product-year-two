...
export default function SuppliesAdmin() {
  ...
  // 🔹 เปิด Popup พร้อมอนิเมชั่น
  const openPopup = (name, currentQty) => {
    setSelectedItem(name);
    setNewQty(currentQty);
    setShowPopup(true);

    // หน่วงเวลาให้ element render ก่อน แล้วค่อย fade-in
    setTimeout(() => setFadeModal(true), 50);
  };

  // 🔹 ปิด Popup พร้อมอนิเมชั่น fade-out
  const closePopup = () => {
    setFadeModal(false);
    setTimeout(() => setShowPopup(false), 300); // ให้เวลาจบ animation
  };
  ...
  return (
    ...
    >
              ...
              {materials.map((item, index) => (
                ...
                >
                  ...
                  <td className="py-2 px-4 border-b text-center">
                    <button
                      className={`h-9 px-4 rounded-2xl font-medium transition transform hover:scale-105 cursor-pointer ${
                        theme === "dark"
      ...
      w-80 p-6 rounded-2xl shadow-xl text-center 
      transition-all duration-500 ease-in-out
      ${fadeModal ? 'scale-100 opacity-100' : 'scale-75 opacity-0'}
